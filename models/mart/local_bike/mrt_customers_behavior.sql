WITH rfm AS (
  SELECT  
    ...
    ROUND(SUM(total_order_amount)/COUNT(DISTINCT order_id),2) AS avg_spend_per_order
...
SELECT
  ...
  CASE 
    ...
    WHEN last_purchase_date > DATE_SUB(md.max_date, INTERVAL 12 MONTH) THEN 4
    WHEN last_purchase_date > DATE_SUB(md.max_date, INTERVAL 18 MONTH) THEN 3
    WHEN last_purchase_date > DATE_SUB(md.max_date, INTERVAL 24 MONTH) THEN 2
    ELSE 1
